    ...
    try:
        ...
        async with clients_lock:
            if client_writer in connected_clients:
                ...
                departing_username = connected_clients[client_writer].get("username", "Someone")
                del connected_clients[client_writer]
                print(f"[Server] User '{departing_username}' ({peername}) removed from active list.")
                # Announce departure only if username was set
                if departing_username != "Someone":
                    await broadcast(f"[Server] '{departing_username}' has left the chat.", client_writer) # Use None as sender so everyone gets it
